...
angular.module('myApp.wasteAreaComparison', ['restangular','ngSanitize','angularSpinner','myApp.eprtrgooglechart'])
   ...
   .controller('WasteAreaComparisonCtrl', ['$scope', '$filter', 'Restangular', 'translationService','formatStrFactory','usSpinnerService', function($scope, $filter,  
        ...
        $scope.$watchCollection('[tr_lovwt,queryparams,wtfilter.wtsel]', function(value){
        	if($scope.queryparams != undefined && $scope.tr_lovwt != undefined && $scope.wtfilter.wtsel ){
        		$scope.startSpin();
    			$scope.getData();
        	}
        });
        ...
        $scope.getData = function(){
		    ...
		    if($scope.wtfilter.wtsel){
		    	qp.WasteType = $scope.wtfilter.wtsel.replace('-','');
		    }
...
//          		$scope.totalSearchResult += parseInt($scope.wastetransfercount);
        ...
        $scope.updateAreaComparisonData = function() {
            ...
            for (var i = 0; i < $scope.items.length; i++) {
                ...
                graphData.push(
                        {c: [{v: gkey},
                             {v: $filter('number')($scope.items[i].pctTotal,2)},
                        	 {v: $filter('number')($scope.items[i].pctTotalAnnexI,2)},
                             {v: $filter('number')($scope.items[i].pctRecovery,2)},
                        	 {v: $filter('number')($scope.items[i].pctRecoveryAnnexI,2)},
                             {v: $filter('number')($scope.items[i].pctDisposal,2)},
                        	 {v: $filter('number')($scope.items[i].pctDisposalAnnexI,2)},
                             {v: $filter('number')($scope.items[i].pctUnspec,2)},
                        	 {v: $filter('number')($scope.items[i].pctUnspecAnnexI,2)}]}
                        );

/*                [ gkey,
                  $filter('number')($scope.items[i].pctTotal,2),
                  $filter('number')($scope.items[i].pctTotalAnnexI,2),
                  $filter('number')($scope.items[i].pctRecovery,2),
                  $filter('number')($scope.items[i].pctRecoveryAnnexI,2),
                  $filter('number')($scope.items[i].pctDisposal,2),
                  $filter('number')($scope.items[i].pctDisposalAnnexI,2),
                  $filter('number')($scope.items[i].pctUnspec,2),
                  $filter('number')($scope.items[i].pctUnspecAnnexI,2)
                ]);*/
  ...
  /*	    data.addColumn('string', title);
  	    data.addColumn('number', $scope.tr_cl["TOTAL"]);
  	    data.addColumn('number', $scope.tr_cl["DOUBLE_COUNTING_TOTAL"]);
  	    data.addColumn('number', $scope.tr_cl["DISPOSAL"]);
  	    data.addColumn('number', $scope.tr_cl["DISPOSAL_COUNTING_TOTAL"]);
  	    data.addColumn('number', $scope.tr_cl["RECOVERY"]);
  	    data.addColumn('number', $scope.tr_cl["RECOVERY_COUNTING_TOTAL"]);
  	    data.addColumn('number', $scope.tr_cl["UNSPECIFIED"]);
  	    data.addColumn('number', $scope.tr_cl["UNSPECIFIED_COUNTING_TOTAL"]);
  	    */
            var chart1 = {};
  	          ...
  	          bars: 'horizontal', // Required for Material Bar Charts.
  				series: {2:{targetAxisIndex:1},3:{targetAxisIndex:1},4:{targetAxisIndex:2},5:{targetAxisIndex:2},6:{targetAxisIndex:3},7:{targetAxisIndex:3}},
  				 hAxes: [{ viewWindow: _range, ticks: _ticks ,format: '#\'%\'', textStyle:{color: 'black'}},
  						{ viewWindow: _range, ticks: _ticks , textStyle:_ts },
  						{ viewWindow: _range, ticks: _ticks , textStyle:_ts},
  						{ viewWindow: _range, ticks: _ticks , textStyle:_ts}]
  	        };
